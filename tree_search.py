...
def tree_descend(tree, amaf_map, disp=False):
    ...
    while nodes[-1].children is not None and passes < 2:
        ...
        node.v += 1
        if node.children is None and node.v >= conf['EXPAND_VISITS']:
            node.expand()
...
def tree_search(tree, n, owner_map, disp=False):
    ...
    while i < n:
        ...
        if len(ongoing) >= n_workers:
            ...
            i += 1
            if i > 0 and i % conf['REPORT_PERIOD'] == 0:
        ...
        if i > n*0.05 and best_wr > conf['FASTPLAY5_THRES'] or i > n*0.2 and best_wr > conf['FASTPLAY20_THRES']:
            break
